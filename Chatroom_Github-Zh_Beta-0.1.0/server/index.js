...
// ===== 性能优化：消息去重和节流 =====
const recentMessages = new Map(); // 存储最近消息的hash，用于去重
const messageThrottle = new Map(); // 消息发送节流
const MESSAGE_THROTTLE_MS = 100; // 最小发送间隔
const MESSAGE_CACHE_TTL = 5000; // 消息缓存过期时间
...
io.on('connection', (socket) => {
  ...
  socket.on('message:private', (data) => {
    ...
    if (isThrottled(senderId)) {
      return socket.emit('message:error', { message: '发送过快，请稍后再试' });
    }
    
    // 性能优化：消息去重
    if (isDuplicateMessage(senderId, data.content, data.type || 'text')) {
      console.log(`[去重] 检测到重复消息: ${sender.nickname}`);
      return;
    }
