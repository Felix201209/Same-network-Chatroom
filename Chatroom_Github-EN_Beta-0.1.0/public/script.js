...
// ===== Beta 0.1.0 Performance Optimization =====
let reconnectAttempts = 0;
const MAX_RECONNECT_ATTEMPTS = 10;
const RECONNECT_DELAY_BASE = 1000;
let isConnected = false;
let pendingMessages = []; // Cached messages when offline
const processedMessageIds = new Set(); // Message deduplication set
let lastMessageTime = 0;
const MESSAGE_DEBOUNCE_MS = 150; // Message send debounce
...
// ===== Socket Initialization (Beta 0.1.0 Optimized Reconnection) =====
function initSocket() {
  socket = io({
    reconnection: true,
  ...
  socket.on('connect', () => {
    ...
    if (pendingMessages.length > 0) {
      console.log(`[Reconnect] Sending ${pendingMessages.length} cached messages`);
  ...
  // Beta 0.1.0: Reconnection event handling
  socket.on('reconnect_attempt', (attemptNumber) => {
    reconnectAttempts = attemptNumber;
    console.log(`[Reconnect] Attempt ${attemptNumber}...`);
    if (attemptNumber > 3) {
      showToast(`Reconnecting (attempt ${attemptNumber})...`, 'warning');
    }
  });
  ...
  socket.on('reconnect_failed', () => {
    ...
    showToast('Cannot connect to server, please check network', 'error');
  });
...
function sendMessage() {
  ...
  if (!isConnected) {
    ...
    showToast('Network disconnected, message will be sent after reconnection', 'warning');
  } else {
