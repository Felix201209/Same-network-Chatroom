...
// Language-specific HTML serving
app.get('/', (req, res) => {
  // Priority: 1. URL param, 2. Cookie, 3. Server default
  let lang = req.query.lang || req.cookies?.chatroom_language || i18n.getLang();
  ...
  const htmlFile = path.join(__dirname, '../public', `index_${lang}.html`);
  
  if (fs.existsSync(htmlFile)) {
    // Set cookie for next visit
    res.cookie('chatroom_language', lang, { maxAge: 365 * 24 * 60 * 60 * 1000 }); // 1 year
...
// Language-specific admin HTML serving
adminApp.get('/', (req, res) => {
  // Priority: 1. URL param, 2. Cookie, 3. Server default
  let lang = req.query.lang || req.cookies?.chatroom_admin_language || i18n.getLang();
  ...
  const htmlFile = path.join(adminDir, `index_${lang}.html`);
  
  if (fs.existsSync(htmlFile)) {
    // Set cookie for next visit
    res.cookie('chatroom_admin_language', lang, { maxAge: 365 * 24 * 60 * 60 * 1000 }); // 1 year
    res.sendFile(htmlFile);
  } else {
    // Fallback to default
...
server.on('error', (err) => {
  if (err.code === 'EADDRINUSE') {
    console.error(`\n❌ ${i18n.getLang() === 'zh' ? '错误' : 'Error'}: ${i18n.getLang() === 'zh' ? '端口' : 'Port'} ${PORT} ${i18n.getLang() === 'zh' ? '已被占用' : 'is already in use'}`);
    console.error(`${i18n.getLang() === 'zh' ? '请运行以下命令清理端口' : 'Please run the following command to clear the port'}:`);
...
adminServer.on('error', (err) => {
  if (err.code === 'EADDRINUSE') {
    console.error(`\n❌ ${i18n.getLang() === 'zh' ? '错误' : 'Error'}: ${i18n.getLang() === 'zh' ? '管理后台端口' : 'Admin port'} ${ADMIN_PORT} ${i18n.getLang() === 'zh' ? '已被占用' : 'is already in use'}`);
    console.error(`${i18n.getLang() === 'zh' ? '请运行以下命令清理端口' : 'Please run the following command to clear the port'}:`);
    console.error(`   lsof -ti:${ADMIN_PORT} | xargs kill -9\n`);
    process.exit(1);
  } else {
